...
public class DotEffector : MonoBehaviour
{
    ...
    public void Shake(Transform _transform)
    {
        ...
        var original = _transform;
        _Sequence.Append(_transform.DOShakePosition(_Duration, _Strange, _Vibrato, _Randomness).SetEase(Ease.InBounce)).
        OnComplete(() => 
        { 
            _transform.position = original.position; 
            _Sequence = null;
        });    
    }
